...
# A variante ${SUPORTE_ATIVO} é o Padrão do Script no Entrypoint.
  ...
  # StartUP comando
    ...
    #  pid=$!
    #  trap 'echo "Aplicativo encerrado. Gravando log..."; cat ./${Nome_egg}.log.txt >> ./${Pasta_Verif}/${Nome_egg}.log.txt; exit' INT
...
# Executa o aplicativo usando o nohup
nohup ${Comando_StartUP} > ${Nome_egg}.log.txt 2>&1 &
pid=$!
...
while true; do
  ...
  # Verifica se o aplicativo ainda está ativo
  if ! kill -0 $pid 2>/dev/null; then
    ...
    if [ $attempts -ge 3 ]; then
      echo "Aplicativo não está respondendo, encerrando script."
      exit 1
    fi

    # Imprime uma mensagem e tenta reiniciar o aplicativo
    echo "Aplicativo fechado, tentando novamente."
    nohup ${Comando_StartUP} > ${Nome_egg}.log.txt 2>&1 &
    pid=$!
  fi
done &
